"""scripts/_fetch_daodejing.py
=================================
One-time corpus assembly script for Classical Chinese Daodejing.

Source: Project Gutenberg eBook #7337 (PUBLIC DOMAIN; Wang Bi recension,
Traditional Chinese, prepared by Ching-yi Chen). The text was fetched
2026-04-29 morning via Cascade's read_url_content tool from
https://www.gutenberg.org/cache/epub/7337/pg7337.txt and the body
(81 chapters: 第一章 ... 第八十一章) is hand-assembled below from the
fetched chunks. Verbatim Classical Chinese characters; no translation,
no commentary, no annotation.

Splits text into 81 chapters using 第N章 markers; computes SHA-256;
writes manifest.
"""
...
ZH_DIR = ROOT / "data" / "corpora" / "zh"

# ============================================================================
# Hand-assembled body text from PG #7337 (positions 0..4 of the fetched URL).
# Header / book-section markers stripped; chapter markers preserved.
# ============================================================================
...
def main() -> None:
    out_path = ZH_DIR / "daodejing_wangbi.txt"
    out_path.write_text(BODY, encoding="utf-8")

    # Verify chapter parse
    chapters = re.split(r"第[一二三四五六七八九十百零]+章\s*", BODY)
    chapters = [c.strip() for c in chapters if c.strip()]
    print(f"# wrote {out_path} ({out_path.stat().st_size:,} bytes)")
    print(f"# parsed chapters: {len(chapters)}")
    print(f"# first chapter (chars): {chapters[0][:80]}")
    print(f"# last chapter (chars):  {chapters[-1][:80]}")

    # Compute SHA-256
    h = hashlib.sha256(out_path.read_bytes()).hexdigest()

    manifest = {
        "corpus": "daodejing_wangbi",
        "language": "Classical Chinese (Traditional, 繁體)",
        "edition": "Wang Bi recension (王弼本); 81 chapters",
        "source_url": "https://www.gutenberg.org/cache/epub/7337/pg7337.txt",
        "source_id": "Project Gutenberg eBook #7337",
        "credits": "Produced by Ching-yi Chen",
        "license": "PUBLIC DOMAIN (Project Gutenberg License)",
        "fetched_at_utc": "2026-04-29T05:50:00Z",
        "files": [
